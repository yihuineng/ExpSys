...
final class TypeConverter {
	...
	private static final String timeStampPattern2 = "yyyy-MM-dd HH:mm";
	...
	public static final Object convert(Class<?> clazz, String s) throws ParseException {
		...
		if (clazz == String.class) {
			return s;	// 原Jfinal用户在表单域中没有输入内容时将提交过来 "", 因为没有输入,所以要转成 null.现改为返回""
		}
		s = s.trim();
		...
		// 经测试java.util.Data类型不会返回, java.sql.Date, java.sql.Time,java.sql.Timestamp 全部直接继承自 java.util.Data, 所以 getDate可以返回这三类数据
        	...
        	} else if (s.length() == timeStampPattern2.length()) {
        		result = new java.sql.Date(new SimpleDateFormat(timeStampPattern2).parse(s).getTime());
        	} else if (s.length() == datePattern.length()) {
