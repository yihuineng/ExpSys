...
import net.yihuineng.framework.reflect.Reflect;
...
public class ClassSearchKit {
	...
	protected static final Log LOG = Log.getLog(ClassSearchKit.class);
	...
	public <T> List<Class<? extends T>> search(String targetPattern) {
		if (includepaths.size() <= 0) {
			...
			while (resources.hasMoreElements()) {
				...
				classFileList.addAll(findPathFiles(resource.getPath(), targetPattern));
			}
			...
			for (String classpath : includepaths) {
				classFileList.addAll(findFiles(classpath, targetPattern));
