package net.yihuineng.framework.easyxls.common;
...
import net.yihuineng.framework.easyxls.bean.Field;
import net.yihuineng.framework.easyxls.generater.GenXml;
...
public class XlsKit {
	...
	public static List<?> xls2List(String xmlPath, File xlsFile)
		...
		try {
			...
			HSSFSheet demoSheet = demoWorkBook
					.createSheet("The World's 500 Enterprises");
			...
			cellStyle
					.setDataFormat(HSSFDataFormat.getBuiltinFormat("0.00E+00"));
	 ...
	 * @throws Exception
	 */
	public static List<?> workbook2List(String xmlPath, Workbook wb)
			throws Exception {
		// 获取配置文件
	 ...
	 * @throws Exception
	 */
	public static List<?> workbook2List(ExcelConfig config, Workbook wb)
			throws Exception {
		...
		int length = sheet.getColumns() < names.length ? sheet.getColumns()
				: names.length;
		// 计算行数
		int rowLength = sheet.getRows() < config.getMaxRow() ? sheet.getRows()
				: (config.getMaxRow() > 0 ? (config.getMaxRow() + config
						.getStartRow()) : sheet.getRows());
		...
		for (int i = config.getStartRow(); i < rowLength; i++) {
			...
			if (key != null) {
				// 当主键为空时，不在继续读取excel
				if (key.get(obj) == null
						|| "".equals(String.valueOf(key.get(obj)))) {
					break;
				}
			}
			list.add(obj);
	...
	private static Object getCellValue(Cell cell) throws Exception {
		 ...
		 * else if (cell instanceof FormulaCell) { value = ((FormulaCell)
		 * cell).getFormula(); }
		 */else {
	 ...
	 * @throws Exception
	 */
	private static void setValue(Object obj, String fieldName, String type,
		...
		if (v == null) {
			...
			String value = "";
			if ("java.lang.Double".equals(v.getClass().getName())) {
				value = decimalFormat.format(v);
			} else {
				value = v.toString();
			}
			if (value != null && !value.trim().equals("")) {
				value = value.trim();
				/**
				 * 对类型进行转换，支持int,long,float,double,boolean,Integer,Long,Double,
				 * Float,Date,String
				 */
				if (type.equals("int")) {
	...
	public static boolean list2Xls(ExcelConfig config, List<?> list,
		...
		try {
			...
			if (!saveFilePath.toLowerCase().endsWith(EXCEL)) {
				saveFilePath += EXCEL;
			}

			WritableWorkbook wb = Workbook.createWorkbook(excelFile);
			String sheetName = (config.getSheet() != null && !config.getSheet()
					.equals("")) ? config.getSheet() : ("sheet" + config
					.getSheetNum());
			...
			if (config.getHeader()) {
				for (column = 0; column < header.length; column++) {
					sheet.addCell(new Label(column, row + rowadd,
							header[column]));
					if (config.getColumn(column).getWidth() != null) {
						sheet.setColumnView(column, config.getColumn(column)
								.getWidth() / 7);
					}
				}
				rowadd++;
	 ...
	 * @throws Exception
	 */
	private static String[] getObjValues(Object source, String... fieldnames)
			throws Exception {
