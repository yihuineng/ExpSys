	...
	$.fn.edatagrid.methods = {
		...
		addRow: function(jq, index){
			return jq.each(function(){
				var dg = $(this);
				var opts = $.data(this, 'edatagrid').options;
				opts.onBeforeAdd.call();//add by yihuineng
				dg = $(this);//再取一次
				var rows = dg.datagrid('getRows');
				if (opts.editIndex >= 0){
				...
				//opts.onBeforeAdd.call(this, opts.editIndex, rows[opts.editIndex]);//add by yihuineng
...
//				}
				...
				opts.onAfterAdd.call(this, opts.editIndex, rows[opts.editIndex]);
			});
		},
		saveRow: function(jq){
			return jq.each(function(){
			...
			return jq.each(function(){
				...
				function _del(row){
					...
					if (row.isNewRecord){
						...
						dg.datagrid('deleteRow', index);//add by yihuineng
						opts.onDestroy.call(dg[0], index, row);//add by yihuineng
					} else {
						if (opts.destroyUrl){
							var idValue = row[opts.idField||'id'];
...
//		destroyConfirmMsg: 'Are you sure you want to delete?',
		...
		onBeforeAdd: function(){},//add by yihuineng 
